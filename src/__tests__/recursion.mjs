...
// A payload deep enough that the unguarded implementation genuinely exhausts
// the call stack rather than merely recursing a lot: ~20k nested pseudos means
// tens of thousands of stack frames on every recursive entry point. Each of the
// four recursive paths reachable from untrusted input — parse, serialize, clone
// and walk — must reject it with a controlled, catchable error.
const EXPLOIT_DEPTH = 20000;

ava('a stack-overflow-scale payload is rejected on every recursive path', t => {
    const parseError = t.throws(
        () => parser().astSync(nest(EXPLOIT_DEPTH)),
        {instanceOf: Error}
    );
    t.false(parseError instanceof RangeError, 'parsing must not overflow the stack');

    // Build the same shape programmatically so the post-parse recursion guards
    // are exercised even though the parse guard would have refused the input.
    const deep = buildDeepAst(EXPLOIT_DEPTH);
    const paths = [
        ['serializing', () => deep.toString()],
        ['cloning', () => deep.clone()],
        ['walking', () => deep.walk(() => {})],
    ];
    for (const [name, run] of paths) {
        const error = t.throws(run, {instanceOf: Error});
        t.false(error instanceof RangeError, `${name} must not overflow the stack`);
    }
});
